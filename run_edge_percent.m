...
acc_100 = zeros(length(folds), 1);
acc_20 = acc_100;
acc_base = acc_100;

for i = 5:length(folds)
    walks2 = walks;
    ...
    %link_mat = link_mat+0.1*ones(size(link_mat));
    restart_prob=0.5;
    c_walk = constraint_walk(link_mat,restart_prob);
    walks2(end+1,:,:) = link_mat;
    x_100 = svd_embed(walks2, options.embedding.ndim);
    
 
    
    walks2 = walks;
    rf = rand_filt > 0.8; % use 1-p fraction of the labels
    filtered_labels = (rf .* training_labels) > 0;
    %link_mat = mustlink(filtered_labels);
    link_mat = mustlink_weighted(filtered_labels, golevels);
    %link_mat = link_mat+0.1*ones(size(link_mat));
    c_walk = constraint_walk(link_mat,restart_prob);
    walks2(end+1,:,:) = link_mat;
    x_20 = svd_embed(walks2, options.embedding.ndim);
    ...
    fprintf('[SMashup] Fold %d / %d \n', i, options.kfolds);
     ...
     dist_mat_base = squareform(pdist(x_base'));
     acc_base(i) = majority_voting_k1_k2(dist_mat_base,k1,k2, anno, test_filt,train_filt);

     %% Perform supervised version
     fprintf('[Perfoming supervised version]\n');

     dist_mat_100 = squareform(pdist(x_100'));
     dist_mat_20 = squareform(pdist(x_20'));
     acc_100(i) = majority_voting_k1_k2(dist_mat_100,k1,k2, anno, test_filt,train_filt);
     acc_20(i) = majority_voting_k1_k2(dist_mat_20,k1,k2, anno, test_filt,train_filt);
...
fprintf('[accuracy_mean_100 = %f ]\n', mean(acc_100));
fprintf('[accuracy_std_100 = %f ]\n', std(acc_100));

fprintf('[accuracy_mean_20 = %f ]\n', mean(acc_20));
fprintf('[accuracy_std_20 = %f ]\n', std(acc_20));
